...
class CarRacingEnv:
    ...
    def _update_checkpoint_progress(self):
        """Update checkpoint progress with strict counter-clockwise direction checking."""
        for env_idx in range(self.num_envs):
            for agent_idx in range(self._n_agents):
                ...
                # Get position and direction vectors
                next_cp_pos = self.checkpoints[next_checkpoint]
                car_position = self.car_positions[env_idx, agent_idx]

                # 1. Check if car is near the next checkpoint
                distance = torch.norm(car_position - next_cp_pos)

                # 2. Verify approaching from the correct direction (previous checkpoint side)
                # In counter-clockwise motion, this means:
                # - If going from CP 0 → 1, we should be coming from the right side
                # - If going from CP 3 → 4, we should be coming from the top-right, etc.

                # Get the previous checkpoint (to determine correct approach vector)
                prev_checkpoint = current_checkpoint
                prev_cp_pos = self.checkpoints[prev_checkpoint]

                # Vector from previous checkpoint to next checkpoint (the ideal path)
                ideal_path = next_cp_pos - prev_cp_pos
                ideal_path = ideal_path / (torch.norm(ideal_path) + 1e-8)  # Normalize

                # Vector from car to next checkpoint
                to_next_cp = next_cp_pos - car_position
                to_next_cp = to_next_cp / (torch.norm(to_next_cp) + 1e-8)  # Normalize

                # Get track direction at this location (tangent to track, counter-clockwise)
                track_direction = self.track_directions[next_checkpoint]

                # Check if car is approaching from correct direction using dot product
                # For counter-clockwise progress: the car should approach from the same general
                # direction as the vector from previous to next checkpoint
                approach_alignment = torch.dot(ideal_path, to_next_cp)

                # 3. Also check alignment with track direction (should be moving along the track)
                car_direction = torch.tensor([
                    torch.cos(self.car_orientations[env_idx, agent_idx]),
                    torch.sin(self.car_orientations[env_idx, agent_idx])
                ], device=self.device)

                track_alignment = torch.dot(car_direction, track_direction)

                # Only count checkpoint if:
                # 1. Close enough (distance check)
                # 2. Approaching from correct direction (alignment > 0)
                # 3. Moving generally in counter-clockwise direction (track_alignment > 0)
                # 4. We haven't just updated this checkpoint (to prevent multiple counts)
                time_since_last_update = self.step_count[env_idx] - self.last_checkpoint_update_step[env_idx, agent_idx]

                if (distance < 30 and
                        approach_alignment > -0.3 and  # Allow some lenience in approach angle
                        track_alignment > 0 and  # Must be going counter-clockwise
                        time_since_last_update > 10):  # Prevent rapid re-triggering

                    # Update checkpoint progress
                    self.checkpoint_progress[env_idx, agent_idx] += 1
                    self.last_checkpoint_update_step[env_idx, agent_idx] = self.step_count[env_idx]
    ...
    def reset(self):
        ...
        # Initialize car positions at the starting line (at checkpoint 0, right side of track)
        ...
        for agent_idx in range(self._n_agents):
            ...
            # Starting position is at checkpoint 0 (right side of track, x-axis)
            # In typical mathematical convention, angle 0 is along the positive x-axis
        ...
        for env_idx in range(self.num_envs):
            for agent_idx in range(self._n_agents):
                # Set orientation to π/2 (90 degrees) which points up
                # This is the correct counter-clockwise direction when starting from the right side
                self.car_orientations[env_idx, agent_idx] = math.pi / 2

        # Add informative print statement to verify orientation during development
        print(
            f"Cars initialized to face counter-clockwise (orientation: {math.pi / 2:.2f} rad = {math.pi / 2 * 180 / math.pi:.1f}°)")

        # Reset velocities and other state variables
        ...
        self.prev_checkpoint_progress = torch.zeros_like(self.checkpoint_progress)
        ...
        self.last_checkpoint_update_step = torch.zeros_like(self.checkpoint_progress)  # For anti-grinding
    ...
    def step(self, tensordict):
        ...
        # Calculate rewards for each agent with anti-reverse and anti-grinding mechanisms
        ...
        for env_idx in range(self.num_envs):
            for agent_idx in range(self._n_agents):
                ...
                prev_checkpoint = self.prev_checkpoint_progress[env_idx, agent_idx]
                ...
                # Get track direction at next checkpoint for directional rewards
                track_direction = self.track_directions[next_cp]

                # Calculate distance to next checkpoint (current and previous)
                current_distance = torch.norm(self.car_positions[env_idx, agent_idx] - next_cp_pos)
                prev_distance = torch.norm(prev_position - next_cp_pos)

                # 1. Direction-aligned reward - MAJOR CHANGE
                # Calculate direction vector to the next checkpoint
                to_checkpoint = next_cp_pos - self.car_positions[env_idx, agent_idx]
                to_checkpoint = to_checkpoint / (torch.norm(to_checkpoint) + 1e-8)

                # Get car's forward vector based on its orientation
                car_direction = torch.tensor([
                    torch.cos(self.car_orientations[env_idx, agent_idx]),
                    torch.sin(self.car_orientations[env_idx, agent_idx])
                ], device=self.device)

                # Alignment between car direction and checkpoint direction
                # Higher when car is facing toward checkpoint
                direction_alignment = torch.dot(car_direction, to_checkpoint)

                # Alignment with track direction (tangential to track)
                # Higher when car is moving along the track in counter-clockwise direction
                track_alignment = torch.dot(car_direction, track_direction)

                # Combined directional reward that rewards both moving toward checkpoint
                # and moving in the counter-clockwise direction of the track
                direction_reward = (direction_alignment * 3.0) + (track_alignment * 5.0)

                # 2. Distance reward - only when actually getting closer
                distance_reward = 0
                if prev_distance > current_distance:
                    distance_reward = (prev_distance - current_distance) * 10.0

                # 3. Strong anti-reverse penalty (major new addition)
                reverse_penalty = 0
                if self.car_speeds[env_idx, agent_idx] < 0:
                    reverse_penalty = -10.0  # Strong penalty for reversing

                # 4. Speed reward - keep appropriate speed range
                # Modified to heavily favor forward movement and penalize stillness
                speed = self.car_speeds[env_idx, agent_idx]
                optimal_speed = 3.0

                # New speed reward design that:
                # - Heavily penalties zero/negative speed
                # - Gives max reward at optimal speed
                # - Slightly reduces reward for too high speed
                if speed <= 0:
                    speed_reward = -5.0  # Strong penalty for not moving forward
                else:
                    # Normalized Gaussian-like reward centered at optimal_speed
                    speed_factor = torch.exp(-0.5 * ((speed - optimal_speed) / 2.0) ** 2)
                    speed_reward = speed_factor * 5.0

                # 5. Checkpoint reward - better scaled and more significant
                checkpoint_reward = 0
                if self.checkpoint_progress[env_idx, agent_idx] > prev_checkpoint:
                    # Give larger rewards for later checkpoints to encourage progress
                    checkpoint_number = prev_checkpoint % self.num_checkpoints
                    checkpoint_reward = 100.0 + checkpoint_number * 10.0  # More significant reward

                # 6. Lap completion reward - huge reward for full lap
                lap_reward = 0
                if self.laps_completed[env_idx, agent_idx] > self.prev_laps_completed[env_idx, agent_idx]:
                    lap_reward = 2000.0  # Massive reward for lap completion

                # 7. Smaller time penalty to encourage exploration
                time_penalty = -0.01

                # 8. Track boundary penalty
                crash_penalty = 0
                if not inside_track[env_idx, agent_idx]:
                    crash_penalty = -30.0  # Increased penalty for going off track

                # Combine rewards with importance-weighted components
                total_reward = (
                        direction_reward +  # Reward for car pointing in right direction
                        distance_reward +  # Reward for getting closer to checkpoint
                        reverse_penalty +  # Strong penalty for reversing
                        speed_reward +  # Reward for maintaining optimal speed
                        checkpoint_reward +  # Larger reward for reaching checkpoints
                        lap_reward +  # Huge reward for completing laps
                        time_penalty +  # Small time penalty
                        crash_penalty  # Penalty for going off track
                )
        ...
        # Store current checkpoint progress and laps for next step comparison
        self.prev_checkpoint_progress = self.checkpoint_progress.clone()
    ...
    def render(self):
        ...
        # Draw checkpoints with improved numbering
        for i, cp in enumerate(self.checkpoints):
            ...
            # Draw checkpoint with larger radius
            pygame.draw.circle(self.screen, (0, 0, 255), (x, y), 8)
        ...
        legend_text = legend_font.render("Checkpoint Sequence: 0 → 1 → 2 → ... → 11 → 0 (Counter-Clockwise)", True,
                                         (0, 0, 0))
