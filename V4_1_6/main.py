...
class CarRacingEnv:
    ...
    def _calculate_checkpoint_direction(self, env_idx, agent_idx):
        """Calculate the direction vector pointing toward the next checkpoint."""
        ...
        direction_vector = checkpoint_pos - car_pos

        # Normalize to unit vector
        direction_length = torch.norm(direction_vector)
        if direction_length > 1e-6:  # Avoid division by zero
            direction_vector = direction_vector / direction_length
        else:
            # If very close to checkpoint, use a default direction (forward)
    ...
    def _calculate_distance_to_checkpoint(self, env_idx, agent_idx):
        ...
        # Calculate distance
        distance = torch.norm(checkpoint_pos - car_pos)

        return distance

    def _is_heading_to_checkpoint(self, env_idx, agent_idx):
        """Determine if the car is heading toward the next checkpoint."""
        # Get car orientation as a unit vector
        car_orient = self.car_orientations[env_idx, agent_idx]
        car_direction = torch.tensor([
            torch.cos(car_orient),
            torch.sin(car_orient)
        ], device=self.device)

        # Get direction to next checkpoint
        checkpoint_direction = self._calculate_checkpoint_direction(env_idx, agent_idx)

        # Calculate dot product (alignment between car direction and checkpoint direction)
        # Values close to 1 mean car is pointing directly at checkpoint
        # Values close to 0 mean car is perpendicular to checkpoint direction
        # Values close to -1 mean car is pointing away from checkpoint
        alignment = torch.dot(car_direction, checkpoint_direction)

        # Also check if the car is actually moving (using speed)
        is_moving = abs(self.car_speeds[env_idx, agent_idx]) > 0.1

        return is_moving and alignment > 0, alignment  # Return both the boolean and the alignment value
    ...
    def _check_checkpoint_reached(self):
        ...
        for env_idx in range(self.num_envs):
            for agent_idx in range(self._n_agents):
                ...
                # Get direction alignment with checkpoint
                _, alignment = self._is_heading_to_checkpoint(env_idx, agent_idx)

                # Check if checkpoint is reached (close enough)
                if distance < 50:  # Distance threshold
                    ...
                    if steps_since_last_update > 5:  # Minimum steps between checkpoint updates
                        ...
                        # Store the previous distance for progress calculation
                        self.prev_checkpoint_distance[env_idx, agent_idx] = self._calculate_distance_to_checkpoint(
                            env_idx, agent_idx)
    ...
    def _calculate_progress_percentage(self, env_idx, agent_idx):
        """Calculate progress percentage based on checkpoints and partial progress to next checkpoint."""
        ...
        current_dist = torch.norm(car_pos - next_cp_pos)
        max_dist = torch.norm(last_cp_pos - next_cp_pos)

        # Calculate fraction of progress between checkpoints
        if total_dist > 0:
            # Progress is better represented by how much we've reduced the distance
            # from the maximum possible distance
            fraction = max(0.0, min(1.0, 1.0 - (current_dist / max_dist)))

            # Check if we're heading toward the checkpoint (not away from it)
            is_heading_to_checkpoint, alignment = self._is_heading_to_checkpoint(env_idx, agent_idx)

            # If not heading to checkpoint, don't count partial progress
            if not is_heading_to_checkpoint:
                fraction = 0.0
    ...
    def _calculate_rewards(self, prev_positions, inside_track):
        """Calculate rewards based on checkpoint progress and heading toward checkpoints."""
        ...
        for env_idx in range(self.num_envs):
            for agent_idx in range(self._n_agents):
                ...
                # 4. DIRECTION REWARD: Encourage heading toward next checkpoint
                direction_reward = 0
                is_heading_to_checkpoint, alignment = self._is_heading_to_checkpoint(env_idx, agent_idx)

                # Scale reward based on alignment with checkpoint direction
                # Higher reward when pointing directly at checkpoint
                if self.car_speeds[env_idx, agent_idx] > 0.5:  # Only when moving at a decent speed
                    direction_reward = alignment * 5.0  # Reward increases with better alignment

                # 5. WRONG DIRECTION PENALTY: Penalize moving away from checkpoint
                wrong_direction_penalty = 0
                if not is_heading_to_checkpoint and self.car_speeds[env_idx, agent_idx] > 0.5:
                    # Penalty scales with how wrong the direction is
                    wrong_direction_penalty = -5.0 * abs(alignment)

                # 6. SPEED REWARD: Encourage maintaining good speed when aligned with checkpoint direction
                speed_reward = 0
                speed = self.car_speeds[env_idx, agent_idx]
                optimal_speed = 3.0

                # Only reward speed when heading in the right direction
                if alignment > 0.5:  # Reasonably aligned with the target direction
                    if speed <= 0:
                ...
                # 8. DISTANCE REWARD: Reward for getting closer to the next checkpoint
                distance_reward = 0
                current_distance = self._calculate_distance_to_checkpoint(env_idx, agent_idx)
                prev_distance = self.prev_checkpoint_distance[env_idx, agent_idx]

                # Reward for reducing distance to checkpoint
                distance_delta = prev_distance - current_distance
                if distance_delta > 0:
                    distance_reward = distance_delta * 0.5  # Small reward for getting closer

                # 9. TIME PENALTY: Small penalty to encourage efficient completion
                time_penalty = -0.1
                ...
                total_reward = (
                        ...
                        direction_reward +  # Reward for facing toward checkpoint
                        wrong_direction_penalty +  # Penalty for moving away from checkpoint
                        speed_reward +  # Reward for good speed
                        distance_reward +  # Reward for getting closer to checkpoint
                ...
                self.prev_checkpoint_distance[env_idx, agent_idx] = current_distance
    ...
    def reset(self):
        """Reset the environment with cars having slightly different starting positions and orientations."""
        # Reset step count
        ...
        # Base starting position - midpoint at positive x-axis (0 degrees)
        mid_radius = (self.track_outer_radius + self.track_inner_radius) / 2
        self.car_positions = torch.zeros(self.num_envs, self._n_agents, 2, dtype=torch.float32, device=self.device)

        # Initialize car orientations to base position facing counter-clockwise (90 degrees or π/2 radians)
        self.car_orientations = torch.ones(self.num_envs, self._n_agents, dtype=torch.float32, device=self.device) * (
                    math.pi / 2)

        # Add small random perturbations to each car to break symmetry
        for env_idx in range(self.num_envs):
            for agent_idx in range(self._n_agents):
                # Base position
                self.car_positions[env_idx, agent_idx, 0] = mid_radius  # x-position
                self.car_positions[env_idx, agent_idx, 1] = 0.0  # y-position

                # Add random position noise (small enough to keep cars on track)
                # The track width is 100, so using noise < 50 keeps cars well within track
                pos_noise_x = (torch.rand(1, device=self.device).item() - 0.5) * 30.0  # -15 to +15 noise
                pos_noise_y = (torch.rand(1, device=self.device).item() - 0.5) * 30.0  # -15 to +15 noise

                # Apply position noise, but make sure car stays well inside track
                self.car_positions[env_idx, agent_idx, 0] += pos_noise_x
                self.car_positions[env_idx, agent_idx, 1] += pos_noise_y

                # Add random orientation noise (small angle variations)
                # +/- 10 degrees (converted to radians)
                orientation_noise = (torch.rand(1, device=self.device).item() - 0.5) * (20.0 * math.pi / 180.0)
                self.car_orientations[env_idx, agent_idx] += orientation_noise
        ...
        self.prev_checkpoint_distance = torch.ones(self.num_envs, self._n_agents, dtype=torch.float32,
                                                   device=self.device) * 1000.0  # Initialize with large distance
        ...
        # Calculate initial observations including checkpoint directions
        ...
        for env_idx in range(self.num_envs):
            ...
            for agent_idx in range(self._n_agents):
                # Get direction to next checkpoint
                checkpoint_dir = self._calculate_checkpoint_direction(env_idx, agent_idx)
                ...
                agent_obs = torch.cat([
                    ...
                    checkpoint_dir,  # Add direction to next checkpoint (2 values: x,y)
        ...
        # Log the random starting positions
        print("Reset with randomized initial positions and orientations:")
        for agent_idx in range(self._n_agents):
            print(f"  Car {agent_idx}: pos=({self.car_positions[0, agent_idx, 0].item():.1f}, "
                  f"{self.car_positions[0, agent_idx, 1].item():.1f}), "
                  f"orientation={self.car_orientations[0, agent_idx].item():.2f} rad")
    ...
    def render(self):
        ...
        title_text = font_large.render("Checkpoint Racing Environment", True, (0, 0, 0))
        ...
        legend_text = legend_font.render("Follow checkpoints in sequence (0 → 1 → 2 → ... → 23 → 0)", True, (0, 0, 0))
        ...
        for agent_idx in range(self._n_agents):
            ...
            # Draw direction arrow showing the direction to the next checkpoint
            # Get direction to next checkpoint
            checkpoint_dir = self._calculate_checkpoint_direction(0, agent_idx)
            # Calculate endpoint
            arrow_length = 30
            target_x = car_pos_x + int(arrow_length * checkpoint_dir[0].item())
            target_y = car_pos_y + int(arrow_length * checkpoint_dir[1].item())

            # Draw line showing direction to target
            pygame.draw.line(self.screen, (0, 255, 0), (car_pos_x, car_pos_y),
                             (target_x, target_y), 2)

            # Draw arrowhead
            arrow_dir_x = checkpoint_dir[0].item()
            arrow_dir_y = checkpoint_dir[1].item()

            pygame.draw.polygon(self.screen, (0, 255, 0), [
                (target_x, target_y),
                (int(target_x - 8 * (arrow_dir_x * 0.7 + arrow_dir_y * 0.7)),
                 int(target_y - 8 * (arrow_dir_y * 0.7 - arrow_dir_x * 0.7))),
                (int(target_x - 8 * (arrow_dir_x * 0.7 - arrow_dir_y * 0.7)),
                 int(target_y - 8 * (arrow_dir_y * 0.7 + arrow_dir_x * 0.7)))
            ])
            ...
            for ray_idx in range(self.num_lidar_rays):
                ...
                # Draw lidar ray
                pygame.draw.line(self.screen, (100, 100, 100), (car_pos_x, car_pos_y), (ray_end_x, ray_end_y), 1)
            ...
            # Show next checkpoint and alignment status
            next_cp = self._get_next_checkpoint_idx(0, agent_idx)
            _, alignment = self._is_heading_to_checkpoint(0, agent_idx)
            ...
            progress_text = progress_font.render(
                ...
                f"cp: {self.last_checkpoint[0, agent_idx].item()} → {next_cp}, "
                f"align: {alignment_str}, "
    ...
    def observation_spec(self):
        ...
        # lidar + position(2) + orientation + speed + checkpoint_direction(2) + checkpoint_count
        obs_dim = self.num_lidar_rays + 2 + 1 + 1 + 2 + 1
